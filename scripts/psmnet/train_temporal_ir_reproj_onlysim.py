...
parser.add_argument(
    "--config-file",
    type=str,
    default="./config_transparent.yaml",
    metavar="FILE",
    help="Config files",
)
parser.add_argument(
...
parser.add_argument(
    "--ps", type=int, default=15, help="Patch size of doing patch loss calculation"
)
...
def train(
    ...
    for epoch_idx in range(cfg.SOLVER.EPOCHS):
        ...
        save_images_grid(
            summary_writer, "val_reproj", img_output_reproj, epoch_idx, nrow=4
        )
        ## Update PSMNet images
...
def train_sample(
    ...
    img_L_transformed, img_R_transformed = transformer_model(
        img_L, img_R
    )  # [bs, 3, H, W]
...
if __name__ == "__main__":
    # Obtain dataloader
    train_dataset = MessytableTransparentDataset(
        cfg.SPLIT.TRAIN,
    ...
    val_dataset = MessytableTransparentDataset(
        cfg.SPLIT.VAL,
        gaussian_blur=args.gaussian_blur,
        color_jitter=args.color_jitter,
        debug=args.debug,
