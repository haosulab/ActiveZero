...
from DispNet.dispnet import DispNet
from DispNet.messytable_test import get_test_loader
...
# python DispNet/test_dispnet.py --config-file ./configs/remote_test.yaml --model /isabella-slow/DispNet/train_1_30/train1_ir_reproj/models/model_133000.pth --output /isabella-slow/DispNet/test_1_31/ --exclude-bg --exclude-zeros --annotate debug --debug --onreal


def test(dispnet_model, val_loader, logger, log_dir):
    dispnet_model.eval()
    ...
    for iteration, data in enumerate(tqdm(val_loader)):
        ...
        with torch.no_grad():
            # _, pref_flow = dispnet_model(img_L, img_R, iters=args.valid_iters, test_mode=True)  # [bs, 1, H, W]
        ...
        pred_disp = (
            pred_disp[:, :, top_pad:, :]
            if right_pad == 0
            else pred_disp[:, :, top_pad:, :-right_pad]
        )
...
def main():
    ...
    os.mkdir(log_dir)
    logger = setup_logger(
        "Reprojection-RAFT Testing", distributed_rank=0, save_dir=log_dir
    )
    ...
    dispnet_model_dict = load_from_dataparallel_model(args.model, "model")
    dispnet_model.load_state_dict(dispnet_model_dict)

    test(dispnet_model, val_loader, logger, log_dir)
