"""
Author: Isabella Liu 1/30/22
Feature: Train ActiveZeroNet with DispNet as backbone
"""
...
from DispNet.dispnet import DispNet, dispnet_loss
from DispNet.messytable_temporal_ir import MessytableDataset
...
# python -m torch.distributed.launch ./DispNet/train_dispnet_temporal_ir_reproj.py --config-file ./RAFT/raft_config.yaml --summary-freq 10 --save-freq 100 --logdir /isabella-slow/RAFT/train_1_30_dispnet/debug --seed 1234 --debug
...
def train(dispnet_model, dispnet_optimizer, TrainImgLoader, ValImgLoader):
    for epoch_idx in range(cfg.SOLVER.EPOCHS):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            # Train one sample
            (
                scalar_outputs_dispnet,
                img_outputs_dispnet,
                img_output_reproj,
            ) = train_sample(sample, dispnet_model, dispnet_optimizer, isTrain=True)

            # Save result to tensorboard
            if (not is_distributed) or (dist.get_rank() == 0):
                scalar_outputs_dispnet = tensor2float(scalar_outputs_dispnet)
                avg_train_scalars_raft.update(scalar_outputs_dispnet)
                if do_summary:
                    ...
                    scalar_outputs_dispnet.update(
                        {"lr": dispnet_optimizer.param_groups[0]["lr"]}
                    )
                    save_scalars(
                        summary_writer,
                        "train_dispnet",
                ...
                if (global_step) % args.save_freq == 0:
                    checkpoint_data = {
                        "epoch": epoch_idx,
                        "model": dispnet_model.state_dict(),
                        "optimizer": dispnet_optimizer.state_dict(),
                    }
                    ...
                    logger.info(
                        f"Step {global_step} train dispnet: {total_err_metric_psmnet}"
                    )
        gc.collect()


def train_sample(sample, dispnet_model, dispnet_optimizer, isTrain=True):
    if isTrain:
        dispnet_model.train()
    else:
        dispnet_model.eval()
    ...
    # Concate input to dispnet
    dispnet_input = torch.cat((img_L, img_R), dim=1)
    if isTrain:
        disp_ests = dispnet_model(dispnet_input)
        sim_pred_disp = disp_ests[0]
        loss_dispnet = dispnet_loss(disp_ests, disp_gt_l, mask)
    ...
    sim_loss = loss_dispnet * args.loss_ratio_sim + sim_ir_reproj_loss

    if isTrain:
        dispnet_optimizer.zero_grad()
        sim_loss.backward()
        dispnet_optimizer.step()
    ...
    dispnet_input = torch.cat((img_real_L, img_real_R), dim=1)
    if isTrain:
        disp_ests = dispnet_model(dispnet_input)
    else:
        with torch.no_grad():
            disp_ests = dispnet_model(dispnet_input)
    real_pred_disp = disp_ests[0]
...
if __name__ == "__main__":
    ...
    # Create DispNet model
    dispnet_model = DispNet().to(cuda_device)
    dispnet_model.weight_bias_init()

    # Create optimizer and lr scheduler
    dispnet_optimizer = torch.optim.Adam(
        dispnet_model.parameters(), lr=cfg.SOLVER.LR_CASCADE, betas=(0.9, 0.999)
    )

    if is_distributed:
        dispnet_model = torch.nn.parallel.DistributedDataParallel(
            dispnet_model, device_ids=[args.local_rank], output_device=args.local_rank
        )
    else:
        dispnet_model = torch.nn.DataParallel(dispnet_model)

    # Start training
    train(dispnet_model, dispnet_optimizer, TrainImgLoader, ValImgLoader)
