...
class BasicMotionEncoder(nn.Module):
    def __init__(self):
        ...
        cor_planes = cfg.MODEL.CORR_LEVELS * (2 * cfg.MODEL.CORR_RADIUS + 1)
...
class BasicMultiUpdateBlock(nn.Module):
    def __init__(self, hidden_dims=[]):
        ...
        self.gru08 = ConvGRU(
            hidden_dims[2],
            encoder_output_dim + hidden_dims[1] * (cfg.MODEL.N_GRU_LAYERS > 1),
        )
        self.gru16 = ConvGRU(
            hidden_dims[1], hidden_dims[0] * (cfg.MODEL.N_GRU_LAYERS == 3) + hidden_dims[2]
        )
