...
parser.add_argument(
    "--output",
    type=str,
    default="../testing_output_feature_cyclegan_psmnet_new",
...
def test(transformer_model, psmnet_model, val_loader, logger, log_dir):
    ...
    for iteration, data in enumerate(tqdm(val_loader)):
        ...
        if args.onreal:
            ...
            with torch.no_grad():
                (
                    img_L_transformed,
                    img_R_transformed,
                    img_L_sim_transformed,
                    img_R_sim_transformed,
                ) = transformer_model(img_L, img_R, img_L_sim, img_R_sim)
        else:
            with torch.no_grad():
                img_real = data["img_L_real"].cuda()  # [bs, 1, H, W]
                img_L_transformed, img_R_transformed, _, _ = transformer_model(
                    img_L, img_R, img_real, img_real
                )
        ...
        with torch.no_grad():
            pred_disp_l, pred_disp_r = psmnet_model(
